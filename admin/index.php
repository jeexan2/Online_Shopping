...
if($_REQUEST['sub'])
 ...
 $sel=mysqli_query($user,"select name,pass FROM details where name='$name'");
 #echo $sel;
...
/*if(isset($_POST['sub'])) {
	$name = $_REQUEST['t1'];
    $pass = $_REQUEST['p1'];
 	$res=mysqli_query($user,"SELECT name,pass FROM details WHERE name='$name'");
 	$row=mysqli_fetch_array($user,$res);
	echo $p;
 if($row['pass']==$pass)
 {
 	  session_start();
  $_SESSION['eid'] = $row['name'];
  header("Location: home.php");
  }
  else
 {
